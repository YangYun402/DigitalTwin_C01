function plotTempField(Temp,node_name)
subnode.s1=[...
    zeros(8,1),(5:12)';
    ones(5,1),(5:9)';
    ones(5,1)*2,(5:9)';
...
for i= 1:length(node)
    ...
    for j=1:size(subnode.(node{i}),1)
        z(22-subnode.(node{i})(j,2),subnode.(node{i})(j,1)+1)=Temp(index);
        z(subnode.(node{i})(j,2)+22,subnode.(node{i})(j,1)+1)=Temp(index);
    end
end
...
colormap("jet");
b=colorbar;
set(b, 'location','south','Limits',[20,80]);
imwrite(frame2im(getframe(gca)),'TF.bmp');
end