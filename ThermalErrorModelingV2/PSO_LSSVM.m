function [gam,sig2] = PSO_LSSVM(train_x,train_y,test_x,test_y)
...
for i=1:P_number
    ...
    P_xy(i,2)=Pbest(i,2);
    Pbest(i,3)=adaptLSSVM(Pbest(i,1),Pbest(i,2),train_x,train_y,test_x,test_y);
    P_xy(i,3)=Pbest(i,3);
...
gbest(1,3)=adaptLSSVM(gbest(1,1),gbest(1,2),train_x,train_y,test_x,test_y);
...
for j=1:K
    for i=1:P_number
        ...
        gam_t=P_xy(i,1);
        sig2_t=P_xy(i,2);
        ad=adaptLSSVM(gam_t,sig2_t,train_x,train_y,test_x,test_y);%评价每个微粒的适应度
        P_xy(i,3)=ad;
    ...
    fprintf('PSO: %d/ %d (%f, %f, %f)\n', j,K,gbest(1,1),gbest(1,2),gbest(1,3));
end
gam=gbest(1,1);
sig2=gbest(1,2);
end